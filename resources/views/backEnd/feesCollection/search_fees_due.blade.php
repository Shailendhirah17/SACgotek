...
@section('mainContent')
    ...
    <section class="admin-visitor-area">
        <div class="container-fluid p-0">
            <div class="row">
                <div class="col-lg-12">
                    <div class="white-box">
                        ...
                        {{ html()->form('POST', route('fees_due_searches'))->attributes([
                        ...
                        <div class="row">
                            ...
                            @if (moduleStatusCheck('University'))
                                ...
                                <div class="col-lg-3 mt-30">
                                    ...
                                    <select
                                        class="primary_select form-control {{ $errors->has('fees_group') ? ' is-invalid' : '' }}"
                                        name="fees_group">
                                        <option data-display="@lang('fees.fees_group')*" value="">@lang('fees.fees_group') *
                                        </option>
                                        @foreach ($fees_masters as $fees_master)
                                            ...
                                            @foreach ($fees_master->feesTypeIds as $feesTypeId)
                                                <option
                                                    value="{{ $fees_master->fees_group_id . '-' . $feesTypeId->feesTypes->id }}"
                                                    {{ isset($fees_group_id) ? ($fees_group_id == $feesTypeId->feesTypes->id ? 'selected' : '') : '' }}>
                                                    {{ $feesTypeId->feesTypes->name }}</option>
                                            @endforeach
                                        @endforeach
                                    </select>
                            ...
                            @else
                                ...
                                <div class="col-lg-3 mt-30-md">
                                    <div class="primary_input">
                                        <input class="primary_input_field" type="number" name="min_due_amount"
                                            value="{{ isset($min_due_amount) ? $min_due_amount : '' }}" placeholder="Amount Greater Than (e.g. 10000)">
                                    </div>
                                </div>
                            @endif
            ...
            {{ html()->form('POST', route('send-dues-fees-email'))->class('form-horizontal')->open() }}
            <div class="row mt-40">
                <div class="col-lg-12">
                    <div class="white-box">
                        <div class="row">
                                        ...
                                        <table id="table_id" class="table " cellspacing="0" width="100%">
                                            ...
                                            <tbody>
                                                @foreach ($fees_dues as $fees_due)
                                                    <tr>
                                                        <td>{{ $fees_due->recordDetail->studentDetail != '' ? $fees_due->recordDetail->studentDetail->admission_no : '' }}
                                                        </td>
                                                        <td>{{ $fees_due->recordDetail->studentDetail != '' ? $fees_due->recordDetail->studentDetail->full_name : '' }}
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            @php
                                                                $balance =
                                                                    discountFeesAmount($fees_due->id) -
                                                                    $fees_due->paid_amount;
                                                                ...
                                                                @if (userPermission('fees_collect_student_wise'))
                                                                    <div class="dropdown-menu dropdown-menu-right">
                                                                        <a class="dropdown-item"
                                                                            href="{{ route('fees_collect_student_wise', [$fees_due->recordDetail->id]) }}">@lang('common.view')</a>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        </td>
                                            ...
                                            <tbody>
                                                @foreach ($fees_dues as $fees_due)
                                                    <tr>
                                                        ...
                                                        <td> {{ $fees_due->recordDetail->studentDetail != '' ? $fees_due->recordDetail->roll_no : '' }}
                                                        </td>
                                                        <td>{{ $fees_due->recordDetail->studentDetail != '' ? $fees_due->recordDetail->studentDetail->full_name : '' }}
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            @php
                                                                $amount = App\SmFeesAssign::discountSum(
                                                                    $fees_due->student_id,
                                                                    $fees_due->feesGroupMaster->feesTypes->id,
                                                                    'amount',
                                                                    $fees_due->recordDetail->id,
                                                                );
                                                                echo $amount;
                                                            @endphp
                                                        </td>
                                                        ...
                                                        <td>
                                                            @php
                                                                echo $fees_due->feesGroupMaster->amount -
                                                                    ...
                                                                    $fine;
                                                                $dues_amount =
                                                                    $fees_due->feesGroupMaster->amount -
                                                                    $discount_amount -
                                                                    $amount;
                                                            @endphp
                                                            <input type="hidden"
                                                                name="dues_amount[{{ $fees_due->recordDetail->id }}]"
                                                                value="{{ $dues_amount }}">
                                                            <input type="hidden" name="student_list[]"
                                                                value="{{ $fees_due->recordDetail->student_id }}">
                                                            <input type="hidden" name="fees_master"
                                                                value="{{ $fees_due->feesGroupMaster->id }}">
                                                        </td>
                                                        <td>
                                                            <div class="dropdown CRM_dropdown">
                                                                <button type="button" class="btn dropdown-toggle"
