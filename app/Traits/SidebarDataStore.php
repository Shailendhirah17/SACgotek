...
trait SidebarDataStore
{
    ...
    function resetSidebarStore($role_name = 'staff')
    {
        ...
        $student_modules_sections = ['tc-list', 'medical-records', 'vaccination-records'];
        $library_book_bank_sections = ['book-bank', 'thirukkural', 'book-bank-issue'];
        $vendor_accounts_sections = ['vendor-list', 'purchase-orders', 'vendor-payments'];
        $hostel_management_sections = ['hostel-list', 'hostel-allocation', 'hostel-fee'];
        ...
        foreach ($permissionInfos as $key => $sidebar) {
            ...
            if (in_array($sidebar->route, $student_modules_sections)) {
                $parent_id = Permission::where('route', 'student_modules_section')
                    ->when($role_name == 'student',function($q){ $q->where('is_student',1); })
                    ->when($role_name == 'parent',function($q){ $q->where('is_parent',1); })
                    ->when($role_name == 'staff',function($q){ $q->where('is_admin',1)->orWhere('is_teacher',1); })
                    ->value('id');
            }
            if (in_array($sidebar->route, $library_book_bank_sections)) {
                $parent_id = Permission::where('route', 'library_book_bank_section')
                    ->when($role_name == 'student',function($q){ $q->where('is_student',1); })
                    ->when($role_name == 'parent',function($q){ $q->where('is_parent',1); })
                    ->when($role_name == 'staff',function($q){ $q->where('is_admin',1)->orWhere('is_teacher',1); })
                    ->value('id');
            }
            if (in_array($sidebar->route, $vendor_accounts_sections)) {
                $parent_id = Permission::where('route', 'vendor_accounts_section')
                    ->when($role_name == 'student',function($q){ $q->where('is_student',1); })
                    ->when($role_name == 'parent',function($q){ $q->where('is_parent',1); })
                    ->when($role_name == 'staff',function($q){ $q->where('is_admin',1)->orWhere('is_teacher',1); })
                    ->value('id');
            }
            if (in_array($sidebar->route, $hostel_management_sections)) {
                $parent_id = Permission::where('route', 'hostel_management_section')
                    ->when($role_name == 'student',function($q){ $q->where('is_student',1); })
                    ->when($role_name == 'parent',function($q){ $q->where('is_parent',1); })
                    ->when($role_name == 'staff',function($q){ $q->where('is_admin',1)->orWhere('is_teacher',1); })
                    ->value('id');
    ...
    function allActivePaidModules()
    {
        ...
        $modules = Cache::rememberForever('paid_modules', function () {
            return InfixModuleManager::where('is_default', false)->where('name', '!=', 'OnlineExam')->pluck('name')->toArray();
        });
