...
class User extends Authenticatable
{
    ...
    protected $fillable = [
        ...
        'role_id', 'school_id', 'active_status', 'full_name'
    ];
    ...
    {
        return $this->hasMany('Modules\Gmeet\Entities\GoogleAccount');
    }

    public function googleAccount(): BelongsTo
    {
        return $this->belongsTo('Modules\Gmeet\Entities\GoogleAccount', 'user_id', 'id');
    }

    public function loginAccount(): BelongsTo
    {
        return $this->belongsTo('Modules\Gmeet\Entities\GoogleAccount', 'user_id', 'id')->where('login_at', 1)->withDefault();
    }
    ...
    {
        return $this->hasMany(SmNotification::class, 'user_id', 'id')->where('academic_id', getAcademicId())->latest();
    }
    ...
    public function generateCode(): void
    {
        try {
            ...
            if ($setting->via_sms && $receiver_No) {
                @send_sms($receiver_No, 'two_factor_code', $compact);
            }
    ...
    {
        return $this->belongsTo(Models\DueFeesLoginPrevent::class, 'id', 'user_id');
    }
