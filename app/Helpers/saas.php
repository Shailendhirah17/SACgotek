...
function activePackage($school = null)
{
    ...
    return Cache::remember('active_package'.$school->id, Carbon::now()->endOfDay()->addSecond(), function () use ($school) {
        ...
        $last_record = Modules\Saas\Entities\SmSubscriptionPayment::with('package')->orderBy('id', 'desc')->where(function ($q): void {
        ...
        $now_time = date('Y-m-d');
        $purchase_packages = Modules\Saas\Entities\SmSubscriptionPayment::with('package')->where('school_id', $school->id)->get();

        $last_active = Modules\Saas\Entities\SmSubscriptionPayment::with('package')->orderBy('id', 'desc')->where('approve_status', 'approved')
