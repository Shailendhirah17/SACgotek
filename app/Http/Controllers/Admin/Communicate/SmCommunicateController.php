...
class SmCommunicateController extends Controller
{
    public function sendEmailSmsView(Request $request)
    {
        ...
        try {
        */
            $roles = InfixRole::select('*')->where('is_saas', 0)->where('id', '!=', 1)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            $classes = SmClass::get();
            $vehicles = SmVehicle::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.communicate.sendEmailSms', ['roles' => $roles, 'classes' => $classes, 'vehicles' => $vehicles]);
        /*
    ...
    public function sendEmailSms(SendEmailSmsRequest $sendEmailSmsRequest)
    {
        ...
        */
            ...
            if ($sendEmailSmsRequest->selectTab == 'B') {
                if (empty($sendEmailSmsRequest->vehicle_id)) {
                    Toastr::error('Please select vehicle you want to send', 'Failed');
                    return redirect()->back();
                }

                if ($sendEmailSmsRequest->send_through == 'E') {
                    $vehicle_ids = $sendEmailSmsRequest->vehicle_id;
                    $to_email = [];
                    $to_mobile = [];
                    
                    foreach ($sendEmailSmsRequest->message_to_vehicle_student_parent as $message) {
                        $student_ids = SmStudent::whereIn('vechile_id', $vehicle_ids)
                            ->where('active_status', 1)
                            ->where('school_id', Auth::user()->school_id)
                            ->pluck('id');
