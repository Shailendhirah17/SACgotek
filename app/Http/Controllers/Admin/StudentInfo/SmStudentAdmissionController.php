...
class SmStudentAdmissionController extends Controller
{
    ...
    public function store(SmStudentAdmissionRequest $smStudentAdmissionRequest)
    {
        ...
        try {
            ...
            $smStudent->emis_id = $smStudentAdmissionRequest->emis_id;
            $smStudent->siblings = $smStudentAdmissionRequest->siblings;
            ...
            if ($smStudentAdmissionRequest->sibling_relation) {
                foreach ($smStudentAdmissionRequest->sibling_relation as $key => $relation) {
                    $sibling = new SmStudentSibling();
                    $sibling->student_id = $smStudent->id;
                    $sibling->sibling_relation = $relation;
                    $sibling->is_studying_in_school = $smStudentAdmissionRequest->is_studying_in_school[$key];
                    if ($sibling->is_studying_in_school == 1) {
                        $sibling->sibling_id = $smStudentAdmissionRequest->sibling_id[$key];
                        $sibling->class_id = $smStudentAdmissionRequest->sibling_class_id[$key];
                        $sibling->section_id = $smStudentAdmissionRequest->sibling_section_id[$key];
                    } else {
                        $sibling->sibling_name = $smStudentAdmissionRequest->sibling_name[$key];
                    }
                    $sibling->school_id = Auth::user()->school_id;
                    $sibling->academic_id = getAcademicId();
                    $sibling->save();
                }
            }
            // end insert
    ...
    public function edit(Request $request, $id)
    {
        ...
        */
            ...
            $data['advanced_siblings'] = \App\SmStudentSibling::where('student_id', $id)->get();
    ...
    public function updateStudentInfo($request, $studentRecord = null)
    {
        ...
        try {
            ...
            $student->emis_id = $request->emis_id;
            ...
            // Advanced Sibling Update
            if (isset($student->id)) {
                \App\SmStudentSibling::where('student_id', $student->id)->delete();
                if ($request->sibling_relation) {
                    foreach ($request->sibling_relation as $key => $relation) {
                        $sibling = new \App\SmStudentSibling();
                        $sibling->student_id = $student->id;
                        $sibling->sibling_relation = $relation;
                        $sibling->is_studying_in_school = $request->is_studying_in_school[$key];
                        if ($sibling->is_studying_in_school == 1) {
                            $sibling->sibling_id = $request->sibling_id[$key];
                            $sibling->class_id = $request->sibling_class_id[$key];
                            $sibling->section_id = $request->sibling_section_id[$key];
                        } else {
                            $sibling->sibling_name = $request->sibling_name[$key];
                        }
                        $sibling->school_id = auth()->user()->school_id;
                        $sibling->academic_id = getAcademicId();
                        $sibling->save();
        ...
        if ($step == 'import') {
            ...
            try {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    $student->emis_id = $request->emis_id[$i] ?? null;
                    $student->siblings = $request->siblings[$i] ?? null;
    ...
    public function ajaxStudentFilter(Request $request)
    {
