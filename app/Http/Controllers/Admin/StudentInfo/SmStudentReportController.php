...
class SmStudentReportController extends Controller
{
    ...
    public function studentMasterReport(Request $request)
    {
        $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get(['class_name', 'id']);
        return view('backEnd.studentInformation.master_report_card', compact('classes'));
    }

    public function studentMasterReportSearch(Request $request)
    {
        $request->validate([
            'class' => 'required',
            'section' => 'required',
        ...
        $student = SmStudent::with('parents', 'gender', 'religion', 'bloodGroup')
            ->where('id', $student_id)
            ->where('school_id', Auth::user()->school_id)
            ->first();

        // 1. Attendance Data
        $attendances = SmStudentAttendance::select('attendance_type', DB::raw('count(*) as total'))
            ->where('student_id', $student_id)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->groupBy('attendance_type')
            ->pluck('total', 'attendance_type')->toArray();

        // 2. Financial Data
        $fees_assigns = \App\SmFeesAssign::with('feesGroupMaster')
            ->where('student_id', $student_id)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get();
        ...
        $total_discount = 0;
        $total_paid = 0;
        foreach($fees_assigns as $assign){
            $total_fees += $assign->feesGroupMaster ? $assign->feesGroupMaster->amount : 0;
            $total_discount += $assign->applied_discount;
            $total_paid += $assign->totalPaid;
        }
        $total_due = $total_fees - ($total_discount + $total_paid);
        $financial = [
        ...
        // 3. Marking / Exam Data
        $marks = \App\SmResultStore::with('exam', 'subject')
            ->where('student_id', $student_id)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get();
        ...
        return view('backEnd.studentInformation.master_report_card', compact('classes', 'student', 'attendances', 'financial', 'marks', 'class_id', 'section_id', 'student_id'));
    }
}
