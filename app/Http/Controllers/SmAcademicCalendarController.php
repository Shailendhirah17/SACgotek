...
class SmAcademicCalendarController extends Controller
{
    ...
    public function calenderData()
    {
        // CalenderQuery Start
            ...
            if (!$roleInfo) {
                // Fail-safe default for broken profiles
                $roleInfo = (object)['id' => $userRoleId, 'name' => 'Guest'];
            }
            ...
            if (isset($admissionQuerySettings->status) && $admissionQuerySettings->status == 1 && ($userRoleId != 2 && $userRoleId != 3)) {
                $admissionQuries = SmAdmissionQuery::get(['name', 'phone', 'email', 'address', 'next_follow_up_date']);
            }

            if (isset($homeworkSettings->status) && $homeworkSettings->status == 1) {
            ...
            if (isset($studyMaterialSettings->status) && $studyMaterialSettings->status == 1) {
            ...
            if (isset($eventSettings->status) && $eventSettings->status == 1) {
            ...
            if (isset($holidaySettings->status) && $holidaySettings->status == 1) {
                $holidays = SmHoliday::where('active_status', 1)->get(['holiday_title', 'details', 'from_date', 'to_date', 'upload_image_file']);
            }

            if (isset($examSettings->status) && $examSettings->status == 1) {
            ...
            if (isset($noticeBoardSettings->status) && $noticeBoardSettings->status == 1) {
            ...
            if (isset($onlineExamSettings->status) && $onlineExamSettings->status == 1) {
            ...
            if (isset($lessonPlanSettings->status) && $lessonPlanSettings->status == 1) {
            ...
            if (isset($leaveSettings->status) && $leaveSettings->status == 1) {
            ...
            if (isset($librarySettings->status) && $librarySettings->status == 1) {
