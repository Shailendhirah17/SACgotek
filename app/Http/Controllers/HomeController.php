...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // DEEP FIX: Only redirect to superadmin-dashboard if user is in the SuperAdmin guard session.
        // School-level admins (role_id=1) who log in via /login should go to admin-dashboard.
        if (($user->role_id == 1) && ($user->is_administrator == 'yes') && (moduleStatusCheck('Saas') == true)) {
            // Check if user is a SuperAdmin (in super_admins table) or a school admin (in users table)
            // School admins log in via /login and should go to admin-dashboard
            if (session()->has('impersonating') || $user->school_id > 1) {
    ...
    public function index(Request $request)
    {
        ...
        $totalStudents = SmStudent::where('active_status', 1)->where('school_id', $school_id)->count();
        $totalParents = SmStudent::where('active_status', 1)->where('school_id', $school_id)->whereNotNull('parent_id')->distinct('parent_id')->count('parent_id');
        $totalTeachers = SmStaff::where('school_id', $school_id)->where('active_status', 1)->where('role_id', 4)->count();
        $totalStaffs = SmStaff::where('school_id', $school_id)->where('active_status', 1)->where('role_id', '!=', 1)->count();
    ...
    public function saveToDoData(Request $request)
    {
        ...
        $results = $toDolists->save();
            ...
            if ($results) {
                ...
                abort(500, 'Fell through ' . ($user ? $user->role_id : 'NoUser') . ' ' . ($usa ? 'USA_Found' : 'No_USA'));
            } else {
    ...
    {
        if (moduleStatusCheck('CustomMenu') && class_exists('\Modules\CustomMenu\Entities\CustomMenu')) {
    ...
    public function masterControl()
    {
        ...
        if ($user && $user->role_id == 10) {
            $usa = \App\Models\UltraSuperAdmin::where('email', $user->email)->first();
            if ($usa) {
                \Illuminate\Support\Facades\Auth::guard('ultrasuperadmin')->login($usa);
