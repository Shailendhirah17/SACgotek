...
class SmAcademicsController extends Controller
{
    ...
    {
        if (moduleStatusCheck('AutomatedTimetable') && (auth()->user()->role_id == 1 || auth()->user()->role_id == 5)) {
            return redirect()->route('automatedtimetable.index');
        }
    ...
    public function universityClassReport($request)
    {
        $input = $request->all();
        $validator = Validator::make($input, [
        ...
        $user = Auth::user();
        $academin_id = getAcademicId();
        if ($request->section !== '') {
            $sectionInfo = SmSection::where('academic_id', $academin_id)->where('id', $request->section)
        ...
        $students = SmStudent::whereIn('id', $student_ids_from_record)->where('school_id', $user->school_id);
        ...
        $assign_class_teacher->where('academic_id',$academin_id)->where('active_status', 1);
        if ($request->section !== '') {
        ...
        $total_collection = SmFeesPayment::where('active_status', 1)->where('academic_id', $academin_id)
                                                                    ->whereIn('student_id', $student_ids)
                                                                    ->whereIn('fees_type_id', $fees_masters)->sum('amount');
        $total_assign = 0;
        $total_due = 0;
        $applied_discount = 0;
        foreach ($students as $student) {
            ...
            foreach ($fees_assigns as $fee_assign) {
                $fees_master = SmFeesMaster::where('academic_id', $academin_id)->where('id', $fee_assign->fees_master_id)->first();    
                $due_date = strtotime($fee_assign->feesGroupMaster->date);
                $now = strtotime(date('Y-m-d'));
                if ($due_date > $now) {
                    continue;
                }    
                $total_assign += $fees_master->amount;
                $total_due += $fee_assign->fees_amount;
                $fees_dues_master_ids[] = $fee_assign->fees_master_id;

            }

            foreach ($fees_dues_master_ids as $fee_due_master_id) {
                $applied_discount += SmFeesAssign::where('student_id', $student->id)->where('fees_master_id', $fee_due_master_id)->sum('applied_discount');
            }
        }

        return view('backEnd.reports.class_report', ['total_due' => $total_due, 'students' => $students, 'applied_discount' => $applied_discount, 'assign_subjects' => $assign_subjects, 'assign_class_teachers' => $assign_class_teachers, 'total_collection' => $total_collection, 'total_assign' => $total_assign,  'sectionInfo' => $sectionInfo, ]);
    ...
    public function classReportSearch(Request $request)
    {
        if (moduleStatusCheck('University')) {
            ...
            $classes = SmClass::where('academic_id', getAcademicId())->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get(['class_name', 'id']);
            $section_id = $request->section;
            $class_id = $request->class;
            $shift_id = shiftEnable() ? $request->shift : '';

            if ($request->shift != "") {
                $shiftInfo = Shift::where('academic_id', getAcademicId())->where('id', $request->shift)->first();
            } else {
                $shiftInfo = '';
            }

            return view('backEnd.reports.class_report', ['total_due' => $total_due, 'classes' => $classes, 'students' => $students, 'applied_discount' => $applied_discount, 'assign_subjects' => $assign_subjects, 'assign_class_teachers' => $assign_class_teachers, 'total_collection' => $total_collection, 'total_assign' => $total_assign, 'search_class' => $search_class, 'sectionInfo' => $sectionInfo, 'section_id' => $section_id, 'class_id' => $class_id, 'shift_id' => $shift_id, 'shiftInfo' => $shiftInfo]);
