...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (count($users) > 0 && count($users) == 1) {
            ...
            if ($user && $user->school_id && $user->school_id != 1) {
                ...
                if (Hash::check($request->password, $user->password)) {
                    // ============================================
                    // DEEP FIX: Direct login for non-subdomain setups
                    // When running on a single domain (no subdomains),
                    // bypass the subdomain redirect and log in directly.
                    // ============================================
                    
                    // Set the correct school context for this user
                    app()->forgetInstance('school');
                    app()->instance('school', $user->school);
                    $request->merge(['school_id' => $user->school_id]);
                    
                    // Set general settings for THIS school
                    $gs = SmGeneralSettings::where('school_id', $user->school_id)->first();
                    ...
                    if (moduleStatusCheck('TwoFactorAuth') && $gs && $gs->two_factor) {
                        $this->twoFactorAuth(auth()->user());
                    }
                    
                    // Fall through to the session setup code below (line ~423 "if ($logged_in)")
                    // by setting the school variable and logged_in flag
                }
            }
        }
        ...
        $logged_in = Auth::check(); // DEEP FIX: May already be true from Single User direct login above
        ...
        if (config('app.app_sync') && $request->auto_login) {
            ...
            if (! $user) {
                $student = SmStudent::where('admission_no', $request->email)->where('school_id', $school->id)->first();
                if ($student) {
                    $user = User::where('id', $student->user_id)->where('school_id', $school->id)->first();
                }
            }
