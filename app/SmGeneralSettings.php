...
class SmGeneralSettings extends Model
{
    ...
    protected $casts = [
        ...
        'shift_enable' => 'integer',
    ];

    public static function getLanguageList()
    {
        try {
            return SmLanguage::all();
    ...
    public static function SUCCESS($redirect_specific_message = null): bool
    {
    ...
    public static function ERROR($redirect_specific_message = null): void
    {
        if ($redirect_specific_message) {
    ...
    public static function make_merit_list($InputClassId, $InputSectionId, $InputExamId)
    {
        try {
            ...
            foreach ($examStudents as $examStudent) {
                $examStudentsids[] = $examStudent->student_id;
            }
            ...
            foreach ($eligible_students as $eligible_student) {
                foreach ($eligible_subjects as $eligible_subject) {
                    $subject_ids[] = $eligible_subject->subject_id;
                    $subject_strings = (empty($subject_strings)) ? $eligible_subject->subject->subject_name : $subject_strings.','.$eligible_subject->subject->subject_name;
                ...
                $full_name = $eligible_student->full_name;                 // get name
                $admission_no = $eligible_student->admission_no;           // get admission no
                $student_id = $eligible_student->id;           // get admission no
                ...
                $insert_results->result = $is_absent == 0 ? $markGrades->grade_name : 'F';
    ...
    public static function isSE($isConfig): bool
    {
        return true;
    }
    ...
    {
        return $this->belongsTo(SmLanguage::class, 'language_id', 'id');
    }

    public function weekStartDay()
    {
        return $this->belongsTo(SmWeekend::class, 'week_start_id', 'id');
    }

    public function dateFormats()
    {
        return $this->belongsTo(SmDateFormat::class, 'date_format_id', 'id');
    }
    ...
    {
        return $this->belongsTo(SmTimeZone::class, 'time_zone_id', 'id')->withDefault();
    }
    ...
    {
        return $this->belongsTo(SmCurrency::class, 'currency', 'code');
    }
