...
// Define exact permission IDs from production
$perms_map = [
...
$routes = array_keys($perms_map);

echo "1. Harmonizing PERMISSIONS globally (school_id = 1)...\n";
foreach ($perms_map as $route => $id) {
    $is_section = str_contains($route, '_section') ? 1 : 0;
    $type = $is_section ? 1 : 2;
    DB::table("permissions")->where("id", $id)->update([
        "school_id" => 1,
        "is_admin" => 1,
        "permission_section" => $is_section,
        "type" => $type,
        "status" => 1,
        "menu_status" => 1
    ]);
...
echo "2. Cleaning up SM_MENUS entries for all Schools...\n";
DB::table("sm_menus")->whereIn("route", $routes)->delete();

echo "3. Re-Syncing SM_MENUS Structure per School...\n";
foreach ($schoolIds as $sid) {
    ...
    foreach ($children as $c) {
        ...
        DB::table('sm_menus')->insert([
            ...
            'status' => 1,
            'menu_status' => 1,
            'permission_id' => $perms_map[$c['route']],
            'created_at' => $ts,
            'updated_at' => $ts,
        ]);
    }
    ...
    // Access Assignments
    foreach ($perms_map as $route => $pid) {
