...
class LessonPlanController extends Controller
{
    ...
    public function searchTeacherLessonPlan($id)
    {
        try {
            $data = $this->loadDefault();
            $data['class_times'] = SmClassTime::where('type', 'class')
    ...
    public function lessonPlannerLesson($day, $teacher_id, $routine_id, $lesson_date)
    {
        ...
        try {
            ...
            $lessons = SmLesson::when(moduleStatusCheck('University'), function ($query) use ($routine): void {
                $query->where('un_semester_label_id', $routine->un_semester_label_id);
            })->when(moduleStatusCheck('University'), function ($query) use ($routine): void {
                $query->where('un_subject_id', $routine->un_subject_id);
            }, function ($query) use ($subject_id): void {
                $query->where('subject_id', $subject_id);
            })
            ->when(! moduleStatusCheck('University'), function ($query) use ($class_id): void {
                $query->where('class_id', $class_id);
            })->when(! moduleStatusCheck('University'), function ($query) use ($section_id): void {
            ...
            return view('lesson::lessonPlan.add_new_lesson_planner_form', ['teacher_id' => $teacher_id, 'lesson_date' => $lesson_date, 'day' => $day, 'class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'teacher_detail' => $teacher_detail, 'lessons' => $lessons, 'routine_id' => $routine_id, 'routine' => $routine]);
        } catch (Exception $exception) {
            \Illuminate\Support\Facades\Log::error($exception->getMessage());
            Toastr::error('Operation Failed: ' . $exception->getMessage(), 'Failed');
            return redirect()->back();
    ...
    public function ViewlessonPlannerLesson($lessonPlan_id)
    {
        try {
            ...
            return view('lesson::lessonPlan.view_lesson_plan', ['lessonPlanDetail' => $lessonPlanDetail, 'teacher_detail' => $teacher_detail]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function EditlessonPlannerLesson($lessonPlan_id)
    {
        try {
            ...
            return view('lesson::lessonPlan.edit_lesson_planner_form', ['lessonPlanDetail' => $lessonPlanDetail, 'topic' => $topic, 'lesson_date' => $lesson_date, 'lessons' => $lessons, 'day' => $day, 'class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'teacher_detail' => $teacher_detail]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function changeWeek(Request $request, $teacher_id, $next_date)
    {
        ...
        $week_number = $end_date->weekOfYear;
        ...
        $teacher_id = $user->role_id === 4 ? SmStaff::where('user_id', $user->id)->first('id')->id : $teacher_id;

        $sm_weekends = SmWeekend::orderBy('order', 'ASC')->get();
        $teachers = SmStaff::where('role_id', 4)->get();

        return view('lesson::lessonPlan.lesson_planner', ['period' => $period, 'dates' => $dates, 'week_number' => $week_number, 'this_week' => $this_week, 'class_times' => $class_times, 'teacher_id' => $teacher_id, 'sm_weekends' => $sm_weekends, 'teachers' => $teachers]);
    }
    ...
    public function topicOverview()
    {
        try {
            ...
            return view('lesson::lessonPlan.manage_lesson', ['topics_detail' => $topics_detail, 'classes' => $classes]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function topicOverviewAjax(Request $request)
    {
        if (! $request->ajax()) {
            ...
            return DataTables::of($topics_detail)
                ->addIndexColumn()
                ->addColumn('topics_name', function ($row): string {
                    $topics_name = '';
                    $topics_title = $row->topics;
                    foreach ($topics_title as $topic_title) {
                        $topics_name .= $topic_title->topic_title;
                        if (($topics_title->last()) !== $topic_title) {
                            $topics_name .= ',';
                        }
                    }
                ...
                ->addColumn('action', function ($row): string {
                    ...
                    <div class="dropdown-menu dropdown-menu-right">'.
                        ...
                        (userPermission('topic-delete') ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="dropdown-item" href="#"  >'.app('translator')->get('common.disable').'</a></span>' :
    ...
    {  
        ...
        try {
            if(moduleStatusCheck('University') ){
                ...
                $lesson_ids = SmLessonTopic::when($un_session_id, function ($q) use ($un_session_id){
                    $q->where('un_session_id', $un_session_id);
                })
                ->when($un_faculty_id, function ($q) use ($un_faculty_id){
                        $q->where('un_faculty_id', $un_faculty_id);
                    })
                ->when($un_department_id, function ($q) use ($un_department_id){
                        $q->where('un_department_id', $un_department_id);
                    })
                ->when($un_academic_id, function ($q) use ($un_academic_id){
                    $q->where('un_academic_id', $un_academic_id);
                })
                ->when($un_semester_id, function ($q) use ($un_semester_id){
                    $q->where('un_semester_id', $un_semester_id);
                })

                ->when($un_semester_label_id, function ($q) use ($un_semester_label_id){
                    $q->where('un_semester_label_id', $un_semester_label_id);
                })
                ->when($un_section_id, function ($q) use ($un_section_id){
                    $q->where('un_section_id', $un_section_id);
                })
                
                ->when($un_subject_id, function ($q) use ($un_subject_id) {
                    $q->where('un_subject_id', $un_subject_id);
                })->get()->pluck('lesson_id')->toArray();

                $topics_detail = SmLessonTopicDetail::whereIn('lesson_id', $lesson_ids)->get();
                return view('lesson::lessonPlan.manage_lesson', ['lessons' => $lessons, 'topics_detail' => $topics_detail]);
                ...
                if (! $class_id && ! $section_id && ! $subject_id) {
                    Toastr::error('Please Select All Field', 'Failed');
                    return redirect()->route('topic-overview');
                }
                $lessons = SmLesson::distinct('lesson_title')->get();
                $classes = SmClass::get();
                $lesson_ids = SmLessonTopic::when($class_id, function ($q) use ($class_id): void {
                ...
                ->when($subject_id, function ($q) use ($subject_id): void {
                        $q->where('subject_id', $subject_id);
                    })
                ->when($lesson_id, function ($q) use ($lesson_id): void {
                ...
                $selected['shift_id'] = shiftEnable() ? $shift_id : null;
                $selected['section_id'] = $section_id;
                $selected['subject_id'] = $subject_id;
                return view('lesson::lessonPlan.manage_lesson', ['lesson_id' => $lesson_id, 'classes' => $classes, 'lessons' => $lessons, 'topics_detail' => $topics_detail, 'class_id' => $class_id, 'shift_id' => $shift_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'selected' => $selected]);
    ...
    public function manageLessonPlanner()
    {
        ...
        try {
            ...
            return view('lesson::lessonPlan.manage_lesson_planner', ['lessonPlanDetail' => $lessonPlanDetail, 'lessons' => $lessons, 'topics' => $topics, 'classes' => $classes, 'teachers' => $teachers]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
        ...
        try {
            ...
            if (moduleStatusCheck('University')) {
                ...
                $alllessonPlanner = LessonPlanner::where('teacher_id', $request->teacher)
                    ...
                    ->get();
            } elseif ($request->teacher !== '' && $request->class_id !== '' && $request->section_id !== '' && $request->subject_id !== '') {
                $lessonPlanner = LessonPlanner::where('teacher_id', $request->teacher)
                    ->where('class_id', $request->class_id)
                    ->where('section_id', $request->section_id)
                    ->where('subject_id', $request->subject_id)
            ...
            $selected['shift'] = $shift_id;

            return view('lesson::lessonPlan.manage_lesson_planner', ['total' => $total, 'completed_total' => $completed_total, 'alllessonPlanner' => $alllessonPlanner, 'lessonPlanner' => $lessonPlanner, 'classes' => $classes, 'teachers' => $teachers, 'percentage' => $percentage, 'subjects' => $subjects, 'sections' => $sections, 'class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id,'shift_id' => $shift_id, 'teacher_id' => $teacher_id, 'selected' => $selected]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function teacherLessonPlanOverview()
    {
        if (Auth::user()->role_id === 4) {
            try {
                ...
                return view('lesson::teacher.teacher_lesson_plan_overview', ['dates' => $dates, 'this_week' => $this_week, 'class_times' => $class_times, 'teacher_id' => $teacher_id, 'sm_weekends' => $sm_weekends, 'teachers' => $teachers]);
            } catch (Exception $e) {
    ...
    public function lessonPlanReport()
    {
        ...
        try {
            ...
            return view('lesson::lessonPlan.report_lesson_plan', ['lessonPlanDetail' => $lessonPlanDetail, 'lessons' => $lessons, 'topics' => $topics, 'teachers' => $teachers]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function searchlessonPlanReport(Request $request)
    {
        try {
            ...
            return view('lesson::lessonPlan.report_lesson_plan', ['total' => $total, 'completed_total' => $completed_total, 'alllessonPlanner' => $alllessonPlanner, 'lessonPlanner' => $lessonPlanner, 'teachers' => $teachers, 'percentage' => $percentage, 'subjects' => $subjects, 'subject_id' => $subject_id, 'teacher_id' => $teacher_id]);
        } catch (Exception $exception) {
        }
    ...
    public function deleteLessonPlanModal($Plan_id)
    {
        try {
            ...
            return view('lesson::lessonPlan.delete_lesson_planner_form', ['id' => $id]);
        } catch (Throwable $throwable) {
            Toastr::error('Operation Failed', 'Failed');
