...
class SmLessonController extends Controller
{
    ...
    public function loadLesson()
    {
        ...
        // Guard: non-teacher admins have no staff record; use empty subject list
        $subjects = $teacher_info
            ? SmAssignSubject::select('subject_id')->where('teacher_id', $teacher_info->id)->get()
            : collect();
        ...
        if (! teacherAccess()) {
            ...
            // Guard: only query if teacher_info exists (has a staff record)
            $data['classes'] = $teacher_info
                ? SmAssignSubject::where('teacher_id', $teacher_info->id)
